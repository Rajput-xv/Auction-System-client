...
function AuctionItem() {
    const { id } = useParams();
    const { isLoggedIn, user: authUser } = useAuth(); // Use the auth context
    ...
    const handleDelete = async () => {
        ...
        try {
            const apiUrl = import.meta.env.VITE_API_URL || "http://localhost:5000";
            await authRequest(`${apiUrl}/api/auctions/${id}`, "DELETE");
            navigate("/auctions");
        } catch (error) {
    ...
    if (!auctionItem) {
        return (
            <div className="max-w-4xl p-8 mx-auto mt-10 text-white bg-gray-900 rounded-lg shadow-lg">
                <p className="text-xl text-center">Auction not found</p>
            </div>
        );
    }
    ...
    return (
        <div className="max-w-4xl p-8 mx-auto mt-10 text-white bg-gray-900 rounded-lg shadow-lg">
            ...
            <p className="mb-4 text-lg">
                Current Highest Bid:{" "}
                <span className="font-semibold">${highestBid || auctionItem.startingBid}</span>
            </p>
            <div
            ...
            {isLoggedIn ? (
                ...
                <div className="p-4 mt-6 text-center bg-gray-800 rounded-lg">
                    <p className="mb-4 text-xl">Please log in to see bids and participate in this auction</p>
                    <Link
                        to="/login"
